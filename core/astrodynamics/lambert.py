...
def lambert_solve(r1_vec, r2_vec, dt, mu, prograde=True, tol=1e-11, max_iter=200):
    ...
    def calc_y(z_val):
        S = stumpff_S(z_val)
        C = stumpff_C(z_val)
        # Handle division by zero for C(z) near zero
        c_denom = np.sqrt(C) if C > 1e-12 else 1e-6
        return r1 + r2 + A * (z_val * S - 1.0) / c_denom

    # 1. Find the lower boundary z_y0 where y(z) = 0
    ...
    if y_zero > 0:
        if y_limit > 0:
            # y(z) remains positive on the entire practical negative domain
            z_y0 = z_limit
        else:
            # y(z) crosses 0 between z_limit and 0.0
            low_z = z_limit
            high_z = 0.0
            z_y0 = z_limit
            for _ in range(40):
    ...
    # 2. Calculate Lagrange coefficients
    f = 1.0 - y / r1
    g = A * np.sqrt(y / mu)
    gdot = 1.0 - y / r2
