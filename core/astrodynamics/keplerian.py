...
def keplerian_to_cartesian(a, e, i, raan, arg_p, nu, mu=MU_SUN):
    ...
    """
    # 1. Position and velocity in the orbital plane (perifocal coordinate system)
    # Avoid division by zero for parabolic/hyperbolic cases
    ...
    # 2. Rotation matrices to convert perifocal to inertial frame
...
def cartesian_to_keplerian(r_vec, v_vec, mu=MU_SUN):
    ...
    if n > 1e-9:
        ...
        if e > 1e-9:
            ...
            arg_p = 0.0  # Circular equatorial orbit
    ...
    if e > 1e-9:
        ...
        # Circular orbit (true anomaly measured from ascending node if inclined)
        if n > 1e-9:
