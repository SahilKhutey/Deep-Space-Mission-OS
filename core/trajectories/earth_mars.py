...
import numpy as np
from core.constants import MU_SUN, AU, DAY_S
...
def get_earth_elements(jd):
    ...
    # Inclination (degrees to radians)
    i = np.radians(-0.00001531 - 0.01294668 * T)

    # RAAN (degrees to radians)
    raan = np.radians(0.0)  # Earth is reference plane

    # Longitude of perihelion (degrees to radians)
    lon_peri = np.radians(102.93768193 + 0.32327364 * T)
    arg_p = lon_peri - raan

    # Mean longitude (degrees to radians)
...
def get_planet_state(planet_name, jd, mu=MU_SUN):
    ...
    if planet_name == "earth":
        ...
        elem = get_planet_state_mars_elem(jd)  # see helper
    else:
...
def get_planet_state_mars_elem(jd):
    # Duplicated helper for elements structure
    return get_mars_elements(jd)
