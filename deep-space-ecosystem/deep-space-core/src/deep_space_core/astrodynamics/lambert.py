"""Lambert's problem solver (universal variables)."""
import numpy as np
...
def stumpff(z):
    """Return both S(z) and C(z) for backward compatibility."""
    return stumpff_S(z), stumpff_C(z)


def lambert_universal(r1, r2, dt, mu, prograde=True, tol=1e-10, max_iter=200):
    """
    Solve Lambert's problem using universal variables.
    
    Parameters
    ----------
    r1 : array_like - Position vector at departure (km)
    r2 : array_like - Position vector at arrival (km)
    dt : float - Time of flight (seconds)
    mu : float - Gravitational parameter (km^3/s^2)
    prograde : bool - Direction of motion
    tol : float - Convergence tolerance
    max_iter : int - Maximum number of iterations
    
    Returns
    -------
    v1 : ndarray - Velocity vector at departure (km/s)
    v2 : ndarray - Velocity vector at arrival (km/s)
    """
    ...
    # To prevent floating-point overflow in sinh/cosh, enforce a limit of -150.0.
    z_limit = -150.0
    ...
    # Recompute y at the final converged z
    y = calc_y(z)
