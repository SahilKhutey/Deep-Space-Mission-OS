...
def fitness_function(params, destination="mars"):
    ...
    try:
        ...
        # Solve Lambert
        v_trans1, v_trans2 = lambert_universal(rE, rD, dt_s, MU_SUN, prograde=True)
...
def optimize_trajectory_ga(bounds, pop_size=40, generations=50, mutation_rate=0.15, destination="mars"):
    ...
    return {
        "best_departure_jd": best_ind[0] if best_ind is not None else 0.0,
        "best_duration_days": best_ind[1] if best_ind is not None else 0.0,
        "best_arrival_jd": (best_ind[0] + best_ind[1]) if best_ind is not None else 0.0,
